...
class CADSynth(Dataset):
    ...
    def load_one_graph(self, file_path):
        ...
        pyg_graph.node_data = graph.ndata["x"].type(FloatTensor)  # node_data[num_nodes, U_grid, V_grid, pnt_feature]12
        pyg_graph.edge_data = graph.edata["x"].type(FloatTensor)  # edge_data[num_edges, U_grid, pnt_feature]12

        pyg_graph.face_type = graph.ndata["z"].type(torch.int)   # face_type[num_nodes]12
        pyg_graph.face_area = graph.ndata["y"].type(torch.float) # face_area[num_nodes]12
        pyg_graph.face_loop = graph.ndata["l"].type(torch.int)   # face_loop[num_nodes]12
        pyg_graph.face_adj = graph.ndata["a"].type(torch.int)    # face_loop[num_nodes]12
        pyg_graph.label_feature = graph.ndata["f"].type(torch.int)  # feature_type[num_nodes]12

        pyg_graph.edge_type = graph.edata["t"].type(torch.int)   # edge_type[num_edges]12
        pyg_graph.edge_len = graph.edata["l"].type(torch.float)  # edge_len[num_edges]12
        pyg_graph.edge_ang = graph.edata["a"].type(torch.float)  # edge_ang[num_edges]12
        pyg_graph.edge_conv = graph.edata["c"].type(torch.int)   # edge_conv[num_edges]12
        ...
        pyg_graph.edge_path = graphfile[1]["edges_path"]           # edge_input[num_nodes, num_nodes, max_dist]1
        pyg_graph.spatial_pos = graphfile[1]["spatial_pos"]        # spatial_pos[num_nodes, num_nodes]12
        pyg_graph.d2_distance = graphfile[1]["d2_distance"]        # d2_distance[num_nodes, num_nodes, 64]12
        pyg_graph.angle_distance = graphfile[1]["angle_distance"]  # angle_distance[num_nodes, num_nodes, 64]12这个就是a3距离
        # 【新增】加载新的节点和图级别属性
        pyg_graph.centroid = graph.ndata["centroid"].type(torch.float)  # 面的质心坐标 (N, 3)
        pyg_graph.curvature = graph.ndata["curvature"].type(torch.int)  # 显式的面曲率 (N,)
        # 将内环和邻接面统计视为浮点数，以便后续通过线性层处理
        pyg_graph.inner_loops = graph.ndata["inner_loops"].type(torch.float)  # 内环详细属性 (N, 2)
        pyg_graph.adj_stats = graph.ndata["adj_stats"].type(torch.float)  # 邻接面分类统计 (N, 6)
        pyg_graph.centroid_distance = graphfile[1]["centroid_distance"]  # 全局面质心距离矩阵 (N, N)
